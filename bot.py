...
def build_week_plain_text(user_id, offset=0):
    """Побудувати текстову статистику тижня для копіювання."""
    ...
    lines = [f"Тиждень {monday.strftime('%d.%m')} – {friday.strftime('%d.%m')}",
             ""]
    total = 0.0
    logged_days = 0
    ...
    for i in range(5):
        ...
        if d_str in logs_by_date:
            ...
            total += net
            logged_days += 1
            lines.append(f"{short} {d.strftime('%d.%m')}  {s} – {e}  (пауза {pause} хв)  {format_hours(net)}")
        elif d > date.today():
            lines.append(f"{short} {d.strftime('%d.%m')}  —")
        else:
    ...
    if logged_days > 0:
        ...
        lines.append(f"Записано днів: {logged_days}")
    else:
...
def build_month_plain_text(user_id):
    """Побудувати текстову статистику місяця для копіювання."""
    now = datetime.now()
    year_month = now.strftime("%Y-%m")
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("SELECT date, start_time, end_time, pause_minutes, net_hours FROM work_log WHERE user_id=? AND date LIKE ? ORDER BY date",
    ...
    month_label = f"{month_name_ua[now.month]} {now.year}"
    work_days_count = sum(1 for d in range(1, calendar.monthrange(now.year, now.month)[1] + 1)
                          if datetime(now.year, now.month, d).weekday() < 5)

    lines = [f"Місяць: {month_label}", ""]
    ...
    total = 0.0
    for date_str, s, e, pause, net in rows:
        d = datetime.strptime(date_str, "%Y-%m-%d").date()
        short = DAYS_SHORT[d.weekday()]
        lines.append(f"{short} {d.strftime('%d.%m')}  {s} – {e}  (пауза {pause} хв)  {format_hours(net)}")
        total += net
    ...
    lines.append("")
    lines.append("──────────────")
    lines.append(f"Записано днів: {days} з {work_days_count} робочих")
    lines.append(f"Всього: {format_hours(total)}")
    lines.append(f"Середньо: {format_hours(avg)} / день")

    remaining = work_days_count - days
    if remaining > 0 and date.today().month == now.month:
        projected = total + remaining * avg
        lines.append(f"Прогноз (якщо avg): {format_hours(projected)}")
...
def main_menu_kb():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("Записати день", callback_data="action:log"),
         InlineKeyboardButton("Редагувати", callback_data="action:edit")],
        ...
        [InlineKeyboardButton("Текст тижня", callback_data="plainweek:0"),
         InlineKeyboardButton("Текст місяця", callback_data="plainmonth")],
    ])
...
def day_select_kb(user_id, mode, week_offset=0):
    ...
    nav = []
    if week_offset > -8:
        nav.append(InlineKeyboardButton("Раніше", callback_data=f"daysnav:{mode}:{week_offset-1}"))
    if week_offset < 0:
        nav.append(InlineKeyboardButton("Пізніше", callback_data=f"daysnav:{mode}:{week_offset+1}"))
    if nav:
        keyboard.append(nav)
...
def pause_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("45 хв", callback_data="pause:45"),
         InlineKeyboardButton("60 хв (стандарт)", callback_data="pause:60"),
         InlineKeyboardButton("90 хв", callback_data="pause:90")],
        [InlineKeyboardButton("Інша кількість хвилин", callback_data="pause:manual")],
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Редагувати ──
        ...
        await query.edit_message_text("*Редагувати запис*\n\nВибери день:",
        ...
        if mode == "delete":
            ...
            if not row:
                ...
                await query.edit_message_text(
                    ...
                    f"`{s}` → `{e}`  пауза `{p} хв`\n"
                    f"Відпрацьовано: *{format_hours(net)}*\n\nЩо змінити?",
    ...
    # ── Поле для редагування ──
        ...
        if field == "delete":
            ...
            prompts = {
                "start": "Введи новий час *початку* (напр. `7:00`):",
                "end": "Введи новий час *закінчення* (напр. `16:00`):",
    ...
    # ── Вибір паузи ──
        ...
        if val == "manual":
            ...
            await query.edit_message_text("Введи кількість хвилин паузи (наприклад `45`):",
                                          parse_mode="Markdown",
                                          reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Скасувати", callback_data="action:menu")]]))
        else:
    ...
    # ── Статистика тижня ──
        ...
        nav = []
        nav.append(InlineKeyboardButton("Попередній", callback_data=f"week:{offset-1}"))
        if offset < 0:
            nav.append(InlineKeyboardButton("Наступний", callback_data=f"week:{offset+1}"))
        kb = InlineKeyboardMarkup([
            nav,
            [InlineKeyboardButton("Скопіювати текст", callback_data=f"plainweek:{offset}")],
            [InlineKeyboardButton("Назад", callback_data="action:menu")]
        ])
    ...
    # ── Статистика місяця ──
        ...
        if not rows:
            ...
            if remaining > 0 and date.today().month == now.month:
                projected = total + remaining * avg
                text += f"Прогноз (якщо avg): *{format_hours(projected)}*"

        await query.edit_message_text(text, parse_mode="Markdown",
                                      reply_markup=InlineKeyboardMarkup([
                                          [InlineKeyboardButton("Скопіювати текст", callback_data="plainmonth")],
                                          [InlineKeyboardButton("Назад", callback_data="action:menu")]
                                      ]))

    # ── Текстова статистика тижня (для копіювання) ──
    elif data.startswith("plainweek:"):
        offset = int(data.split(":")[1])
        plain = build_week_plain_text(user_id, offset)
        monday, friday = get_week_range(offset)
        nav_btns = []
        nav_btns.append(InlineKeyboardButton("Попередній тиждень", callback_data=f"plainweek:{offset-1}"))
        if offset < 0:
            nav_btns.append(InlineKeyboardButton("Наступний тиждень", callback_data=f"plainweek:{offset+1}"))
        await query.edit_message_text(
            f"```\n{plain}\n```",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
    ...
    # ── Текстова статистика місяця (для копіювання) ──
    elif data == "plainmonth":
        plain = build_month_plain_text(user_id)
        await query.edit_message_text(
            f"```\n{plain}\n```",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "start":
        t = parse_time(text)
        if not t:
            await update.message.reply_text("Формат: `7:00` або `7.30`", parse_mode="Markdown")
            return
        context.user_data["log_start"] = text
        context.user_data["step"] = "end"
        await update.message.reply_text(
            f"Початок: `{text}`\n\nТепер введи час *закінчення* (напр. `16:00`):",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Скасувати", callback_data="action:menu")]]))
    ...
    # ── Введення кінця ──
        ...
        if not t:
            await update.message.reply_text("Формат: `16:00`", parse_mode="Markdown")
            return
        if s and t <= s:
    ...
    # ── Редагування поля ──
        ...
        if field in ("start", "end"):
            t = parse_time(text)
            if not t:
                await update.message.reply_text("Формат: `7:00`", parse_mode="Markdown")
                return
        ...
        await update.message.reply_text(
            ...
            f"`{row[0]}` → `{row[1]}`\n"
            f"Пауза: `{row[2]} хв`\n"
            f"Відпрацьовано: *{format_hours(row[3])}*",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Меню", callback_data="action:menu")]]))
