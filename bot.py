...
def main_menu_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📤 Звіт тижня", callback_data="plainweek:0"),
         InlineKeyboardButton("📤 Звіт місяця", callback_data="plainmonth")],
    ])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"👋 Привіт{name}!\n\n"
        f"🕐 *hrs · wiedzmin*\n"
        f"_Твій особистий облік робочих годин_\n\n"
        f"Записуй робочі дні, переглядай статистику тижня та місяця, "
        f"та експортуй звіти у текстовому форматі.\n\n"
        f"Обери дію 👇"
    )
    kb = main_menu_kb()
    if update.message:
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Статистика тижня ──
        ...
        kb = InlineKeyboardMarkup([
            nav,
            [InlineKeyboardButton("📤 Скопіювати звіт", callback_data=f"plainweek:{offset}")],
            [InlineKeyboardButton("🔙 Назад", callback_data="action:menu")]
        ])
    ...
    # ── Статистика місяця ──
        ...
        await query.edit_message_text(text, parse_mode="Markdown",
                                      reply_markup=InlineKeyboardMarkup([
                                          [InlineKeyboardButton("📤 Скопіювати звіт", callback_data="plainmonth")],
                                          [InlineKeyboardButton("🔙 Назад", callback_data="action:menu")]
                                      ]))
    ...
    # ── Текстова статистика тижня (для копіювання) ──
        ...
        monday, friday = get_week_range(offset)
        nav_btns = []
        nav_btns.append(InlineKeyboardButton("◀️ Попередній тиждень", callback_data=f"plainweek:{offset-1}"))
        if offset < 0:
            nav_btns.append(InlineKeyboardButton("▶️ Наступний тиждень", callback_data=f"plainweek:{offset+1}"))
        await query.edit_message_text(
            f"```\n{plain}\n```",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "start":
        t = parse_time(text)
        if not t:
            await update.message.reply_text("❌ Невірний формат. Введи час як `7:00`", parse_mode="Markdown")
            return
    ...
    # ── Введення кінця ──
        ...
        if not t:
            await update.message.reply_text("❌ Невірний формат. Введи час як `16:00`", parse_mode="Markdown")
            return
        if s and t <= s:
